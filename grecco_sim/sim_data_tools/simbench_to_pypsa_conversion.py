...
class SimbenchToPypsa:
    ...
    def __init__(self, sb_code="1-LV-semiurb4--2-no_sw"):
        ...
        self.profiles = simbench.get_absolute_values(
            self.net_simbench, profiles_instead_of_study_cases=True
        )
    ...
    def simbench_to_pypsa(self):
        ...
        for index, bus in self.net_simbench.bus.iterrows():
            self.net_pypsa.add(
                "Bus",
                name=str(self.net_simbench.bus_geodata.index[index]),
                v_nom=bus["vn_kv"],
                x=self.net_simbench.bus_geodata.loc[index, "x"],
                y=self.net_simbench.bus_geodata.loc[index, "y"],
                sub_network=bus["subnet"],
                v_mag_pu_max=bus["max_vm_pu"],
        ...
        for index, line in self.net_simbench.line.iterrows():
            self.net_pypsa.add(
                ...
                num_parallel=line["parallel"],
                s_nom=math.sqrt(3) * line["voltLvl"] * line["max_i_ka"],
        ...
        for index, sgen in self.net_simbench.sgen.iterrows():
            self.net_pypsa.add(
                ...
                p_set=sgen["p_mw"],
                q_set=sgen["q_mvar"],
                carrier="solar" if sgen["type"] == "PV" else sgen["type"],
            )
        ...
        for index, trafo in self.net_simbench.trafo.iterrows():
            self.net_pypsa.add(
                "Transformer",
                name=str(index),
                bus0=str(trafo["hv_bus"]),
                bus1=str(trafo["lv_bus"]),
                phase_shift=trafo["shift_degree"],
                s_nom=trafo["sn_mva"],
                tap_side=0 if trafo["tap_side"] == "hv" else 1 if trafo["tap_side"] == "lv" else 0,
                tap_position=trafo["tap_pos"],
                num_parallel=trafo["parallel"],
                sub_network=trafo["subnet"],
                tap_ratio=trafo["tap_step_percent"] / 100,
        ...
        if self.profiles[("gen", "p_mw")].empty and not self.profiles[("sgen", "p_mw")].empty:
            ...
            self.net_pypsa.generators_t["p_set"] = (
                self.profiles[("sgen", "p_mw")] + self.profiles[("gen", "p_mw")]
            )
