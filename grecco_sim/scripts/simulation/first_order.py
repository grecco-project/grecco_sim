...
# Comment out if you want to analyse periods of highes congestion in trafo.
# These cases are nevertheless interesting to analyse
SELECTED_CASES_AND_P_LIM = [
    # (datetime.datetime(2023, 1, 1, 0, 0, 0, tzinfo=pytz.utc), 70),
    (datetime.datetime(2023, 12, 11, 0, 0, 0, tzinfo=pytz.utc), 65),
...
def run_congestion_cases(run_name: str):
    ...
    cm_solver = [
        ("plain_grid_fee", "gurobi", 1),
        # ("plain_grid_fee", "bonmin"),
        ("admm", "gurobi", 2),
        ("admm", "gurobi", 5),
        ("admm", "gurobi", 8),
        ("local_self_suff", "", 1),
        # ("none", ""),
        ("central_optimization", "gurobi", 1),
    ]
    ...
    for date, p_lim in SELECTED_CASES_AND_P_LIM:
        ...
        for cm, solver, max_iter in cm_solver:

            sim_tag = f"{run_name}_{cm}_{max_iter}_{date.strftime('%m%d')}"

            run_pars += [
                type_defs.RunParameters(
                    sim_horizon=96,
                    start_time=date,
                    max_market_iterations=max_iter,
