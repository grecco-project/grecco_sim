...
def plot_grid_powers(
    flex_sum: np.ndarray, inflex_sum: np.ndarray, y_static: np.ndarray, constr_val, tol=1e-3
):
    fig, ax = style.styled_plot(
        figsize="landscape", title=f"Constraint: {constr_val: 0.04f}", ylabel="Grid Power / kW"
    )
...
def plot_sec_order_ref_power(ref_power_grid, inflex_sum, flex_sum, mul_lam):
    ...
    ax.plot(mul_lam * 100.0, label="lambda")
    fig.legend()
